"""
    QA generation via GPT4V
    input consist of 'system message' + 'few shot sample' + 'query context'
    'system message' -> role:System
    'few shot sample' -> role:User & assistant
    'query context' -> role: User
"""
import base64
import os
import requests
...
def system_msg_generator(general_prompt, qa_instruction):
    """
        system message differ by question category
        role of system message is "system"
    """
    sys_msg = {
        "role" : "system",
        "content" : 
            {
                "type" : "text",
                "text" : f"{general_prompt} {qa_instruction}"
            }
    }
...
def few_shot_loader():
    """
        generate 3 few shot sample of qa including diverse furniture type.
        question종류에 따라 알맞은 few shot sample이 필요함 
    """
