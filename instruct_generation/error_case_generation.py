...
def main(args):
    print(f"QA generation Error cases {args.mode}, save path is {args.save_path}")

    # error 후보군 모집 
    base_path = "/data/generate/qa_annot/single_turn"
    error_case = f"qa_response_cat{args.category_type}_error_{args.api_num}.json"

    with open(os.path.join(base_path, error_case), 'r') as f:
        error_candidate = json.load(f)

    #collect serial nums
    query_list = []
    for k, v in error_candidate.items():
        query_list.append(k)
    ...
    api_key = ingredients[f"API_{args.api_num}"]["api"] #api key만 필요하니 key만 빼오기
