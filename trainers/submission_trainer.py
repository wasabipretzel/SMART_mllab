...
from transformers.trainer_pt_utils import find_batch_size, is_torch_xla_available, EvalLoopContainer, nested_numpify
...
class EvalPrediction:
    ...
    def __init__(
        ...
        keyval_class: Optional[Union[np.ndarray, Tuple[np.ndarray]]] = None,
    ):
        self.predictions = predictions
        self.label_ids = label_ids
        self.inputs = inputs
        self.keyval_class = keyval_class

    def __iter__(self):
        if self.inputs is not None:
            return iter((self.predictions, self.label_ids, self.inputs))
        else:
            return iter((self.predictions, self.label_ids, self.keyval_class))
    ...
    def __getitem__(self, idx):
        ...
        if idx == 2 and self.inputs is None:
            raise IndexError("tuple index out of range")
        if idx == 3 and self.keyval_class is None:
            raise IndexError("tuple index out of range")
        if idx == 0:
            ...
            return self.keyval_class

class SubmissionTrainer(Seq2SeqTrainer):
    ...
    def evaluation_loop(
        ...
        all_keyval_class = EvalLoopContainer(do_nested_concat=False, padding_index=-100)
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels, keyval_class = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            # breakpoint()
            ...
            if keyval_class is not None:
                keyval_class = self.gather_function((keyval_class))
                all_keyval_class.add(keyval_class)
            ...
            if args.eval_accumulation_steps is not None and (step + 1) % args.eval_accumulation_steps == 0:
                ...
                all_keyval_class.to_cpu_and_numpy()
        ...
        all_keyval_class = [each_class[0] for each_class in all_keyval_class.tensors]
        ...
        if self.compute_metrics is not None and all_preds is not None and all_labels is not None:
            if args.include_inputs_for_metrics:
                ...
                metrics = self.compute_metrics(EvalPrediction(predictions=all_preds, label_ids=all_labels, keyval_class=all_keyval_class))
        else:
            metrics = {}
    ...
    def prediction_step(
        ...
        **gen_kwargs,
        ...
        return loss, generated_tokens, labels, outputs["keyval_class"]