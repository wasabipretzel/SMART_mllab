...
from metrics.submission_criterion import SubmissionCriterionAnswerKey, SubmissionCriterionAnswerValue, SubmissionCriterionAnswerAll
    ...
    if "instructblip" in model_args.model_type:
        if data_args.challenge_phase != None:
            if data_args.prediction_type == "answerkey":
                return SubmissionCriterionAnswerKey(tokenizer=processor.tokenizer, vicuna_embedding=embeddings, eval_dataset=eval_dataset, puzzle_path=data_args.puzzle_path)
            elif data_args.prediction_type == "answervalue":
                return SubmissionCriterionAnswerValue(tokenizer=processor.tokenizer, vicuna_embedding=embeddings, eval_dataset=eval_dataset, puzzle_path=data_args.puzzle_path)
            ...
            return ComputeMetricAnswerAll(tokenizer=processor.tokenizer, vicuna_embedding=embeddings, eval_dataset=eval_dataset, puzzle_path=data_args.puzzle_path)
