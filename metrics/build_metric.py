...
from metrics.submission_criterion import SubmissionCriterionAnswerKey, SubmissionCriterionAnswerValue, SubmissionCriterionEnsemble
...
def get_metric(model_args, data_args, processor, embeddings, eval_infos):
    ...
    if data_args.challenge_phase != None:
        ...
        elif data_args.prediction_type == "ensemble_classify_category":
            return SubmissionCriterionEnsemble(tokenizer=processor.tokenizer, vicuna_embedding=embeddings, eval_infos=eval_infos, puzzle_path=data_args.puzzle_path)
        else:
